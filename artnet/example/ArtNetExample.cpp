...
void myDataCallback(uint16_t universe, const uint8_t *data, uint16_t length)
{
	std::cout << "myDataCallback: Received DMX data on universe: " << universe
			<< ", length: " << length << ", data: ";
...
void printUsage(const char *programName)
{
	std::cout << "Usage: " << programName << " [options]\n" << "Options:\n"
			...
			<< "Examples:\n" << "  " << programName
			<< " --bind=192.168.1.100 --broadcast=192.168.1.255\n" << "  "
...
int main(int argc, char *argv[])
{
	...
	{
		ArtNet::Logger::info(
				"Failed to set process priority. Try running with sudo.");
	}
	...
	if (!controller.configure(config.bindAddress, config.port, config.net,
			config.subnet, config.universe, config.broadcastAddress))
	{
	...
	ArtNet::Logger::info("Controller running at ", ArtNet::ARTNET_FPS,
			" FPS. Press Ctrl+C to exit.");
	...
	while (running)
	{
		...
		{
			std::cout << "\rFrames: " << stats.totalFrames << " | Queue: "
					<< stats.queueDepth << " | Dropped: " << stats.droppedFrames
