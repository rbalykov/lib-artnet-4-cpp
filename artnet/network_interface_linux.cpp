...
bool NetworkInterfaceLinux::createSocket(const std::string &bindAddress, int port) {
  ...
  if (setsockopt(m_socket, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop)) < 0) {
    std::cerr << "ArtNet: Failed to set socket to allow multicast loopback" << std::endl;
    return false;
  }
...
bool NetworkInterfaceLinux::bindSocket() {
  ...
  if (bind(m_socket, reinterpret_cast<sockaddr *>(&addr), sizeof(addr)) == -1) {
    std::cerr << "ArtNet: Error binding socket to address: " << m_bindAddress << ":" << m_port << ". " << strerror(errno) << std::endl;
    return false;
  }
  return true;
}

bool NetworkInterfaceLinux::sendPacket(const std::vector<uint8_t> &packet, const std::string &address, int port) {
  if (m_socket == -1) {
  ...
  if (bytesSent == -1) {
    std::cerr << "ArtNet: Error sending packet: " << strerror(errno) << std::endl;
    return false;
  }
  return true;
}
...
int NetworkInterfaceLinux::receivePacket(std::vector<uint8_t> &buffer) {
  ...
  if (bytesReceived == -1) {
    if (errno != EAGAIN && errno != EWOULDBLOCK) {
      std::cerr << "ArtNet (Linux): Error receiving data: " << strerror(errno) << std::endl;
    }
